...
2. `한국자산관리공사_온비드 물건 정보 조회서비스`를 검색해서 **활용신청**을
   누른다 (무료, 개발계정 기준 일 약 1,000건 트래픽). 이 프로젝트의
   `onbid_source.py`는 `PublicDataReader.Kamco`를 통해 이 서비스의
   `통합용도별물건목록`(`ThingInfoInquireSvc/getUnifyUsageCltr`) 기능을
   호출한다 — data.go.kr에 "차세대"라는 이름이 붙은 유사 서비스도 있는데,
   그건 다른 엔드포인트라 지금 코드와는 맞지 않으니 신청하지 않아도 된다.
...
- `onbid_source.py`의 필드 매핑은 로컬에 설치된 `PublicDataReader==1.1.1-post2`의
  내부 컬럼 번역 테이블을 정적으로 확인해서 작성했지만, 실제 온비드 서버에
  요청을 보내 응답을 받아 end-to-end로 검증하지는 못했다 (개발 환경의
  네트워크 정책상 `openapi.onbid.co.kr`로 나가는 아웃바운드가 차단됨). 서비스키
  발급 후 처음 실행할 때 실제 응답과 필드가 다르면 `_row_to_item`을 조정하자.
