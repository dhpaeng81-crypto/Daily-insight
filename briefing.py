...
def generate_content(news_list):
    ...
    news_text = ""
    for n in news_list:
        news_text += f"[{n['category']}] {n['title']}\n{n['summary']}\n\n"

    prompt = f"""
아래 뉴스를 바탕으로 Daily Insight 웹진 콘텐츠를 JSON 형식으로 작성해줘.
독자는 투자자와 일반인이야. 전문용어는 쉽게 설명하고, 왜 중요한지 맥락을 포함해줘.

반드시 아래 JSON 형식으로만 응답해 (다른 텍스트 없이):
{{
  "hero_title": "오늘의 핵심 헤드라인 (20자 이내)",
  "hero_desc": "오늘 브리핑 전체를 관통하는 한 줄 요약 (50자 이내)",
  "finance_overview": "금융 시장 전반 흐름 2-3문장",
  "finance_comment": "투자자 관점 인사이트 1-2문장",
  "tech_overview": "AI/IT 트렌드 전반 흐름 2-3문장",
  "tech_comment": "IT/투자 관점 인사이트 1-2문장",
  "energy_overview": "에너지 시장 전반 흐름 2-3문장",
  "energy_comment": "에너지/투자 관점 인사이트 1-2문장",
  "key_insight_1": "오늘의 핵심 인사이트 1 (30자 이내)",
  "key_insight_2": "오늘의 핵심 인사이트 2 (30자 이내)",
  "key_insight_3": "오늘의 핵심 인사이트 3 (30자 이내)",
  "news_summaries": [
    {{
...
.hero-desc {{ font-size: 16px; color: var(--ink-soft); max-width: 560px; margin-bottom: 24px; }}
...
<main class="main">
  ...
  <div class="summary-box">
    <div class="summary-label">Today's Key Insights</div>
